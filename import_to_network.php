<?php
	...
	foreach ($single_db->get_results("select * from wp_users", ARRAY_A) as $user) {
		...
		foreach ($metadatas as $metadata) {
            if($metadata->meta_key == 'wp_capabilities'){
				add_user_meta($user_id, "wp_".$new_blog_id."_capabilities",
					maybe_unserialize($metadata->meta_value, true)) ;
            } else {
	...
	$new_user_roles_option_name = "wp_".$new_blog_id."_user_roles" ;
	$single_db->query("update $new_wp_options_table set option_name = '$new_user_roles_option_name' where option_name = 'wp_user_roles' ;") ;
